...
export class AnthropicService {
    ...
    async #getNextResponse(scopedUserDataManager, nextMessage) {
        ...
        const options = {
            method: 'POST',
            headers: this.#getDefaultHeaders(effectiveApiKey),
        ...
        try {
            ...
            if (logger.checkLogLevel(Logger.LEVEL_DEBUG)) {
                logger.debug(`Anthropic response: ${JSON.stringify(data)}`);
            }
    ...
    async #handleChatCompletionResponse(scopedUserDataManager, data) {
        if (data?.stop_reason === "tool_use") {
            return this.#handleToolCalls(scopedUserDataManager, data.content, async () => {
                return this.#getNextResponse(scopedUserDataManager);
            });
        }

        const messageContents = this.#parseMessageContent(data.content)
     ...
     * @return {Promise<*>}
     */
    async #handleToolCalls(scopedUserDataManager, content, callback) {
            ...
            toolCallRequests.push(tool.internal.execute(contentElement.input)
                .then(responseJson => {
                    if (responseJson.error) {
                        return new FunctionCallResultContent(contentElement.id, responseJson.error);
                    }

                    const filteredResponse = tool.internal.getFilteredResponse(responseJson);
                    return new FunctionCallResultContent(contentElement.id, JSON.stringify(filteredResponse));
                }));
        });
    ...
    async #getApiKey(scopedUserDataManager) {
        ...
        if (!userApiKey) {
            logger.warn(`Using default Anthropic API key`);
        }
     ...
     *
     * @param apiKey {string}
     * @return {{"anthropic-version": string, "Content-Type": string, "x-api-key": string}}
     */
